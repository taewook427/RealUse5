...
func (tbox *langre) load(path string) error {
	...
	tbox.lib.Init(slices.Contains(tbox.abi_code, 2), slices.Contains(tbox.abi_code, 4), slices.Contains(tbox.abi_code, 8), tv.Dat6 == "windows")
...
func (tbox *langre) run() error {
	...
	usetlib := slices.Contains(tbox.abi_code, 1)
	for {
