...
class img_resize:
    ...
    def entry(self):
        ...
        def save_img(): # save image
            time.sleep(0.1)
            x, y, path, temp = self.comp["entry4a"].get(), self.comp["entry4b"].get(), self.comp["entry3"].get() + "." + self.comp["combo3"].get(), self.img_object
            if x != "" and y != "":
                temp = temp.resize( ( int(x), int(y) ) )
            temp.save("./_ST5_DATA/" + path, lossless=True)
        ...
        self.comp["combo3"] = tkt.Combobox( self.comp["frame3"], font=self.parms["font.1"], width=self.parms["combo.3"], values=["jpg", "png", "bmp", "webp", "ico", "Manual"] )
    ...
    def entry(self):
        ...
        def save_file(): # save file
            ...
            if mode == "img -> webp":
                ...
                for i in range( 0, len(self.path) ):
                    try:
                        Image.open( self.path[i] ).save("./_ST5_DATA/" + name[i] + ".png", lossless=True)
                ...
                for i in range( 0, len(self.path) ):
                    try:
                        Image.open( self.path[i] ).save("./_ST5_DATA/" + name[i] + ".jpg", lossless=True)
        ...
        self.comp["combo1"] = tkt.Combobox( self.comp["frame1"], font=self.parms["font.1"], width=self.parms["c"],
            values=["img -> webp", "img -> png", "img -> jpg", "html -> pdf", "merge img", "merge mp4", "merge_cp mp4", "merge pdf"] )
