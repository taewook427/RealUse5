...
class genbytes: # kdm용 일반 모드 - bytes
    ...
    def en(self, pw, kf, hint, data):
        ...
        pwhash = self.assist.svkey(salt, pw, kf) # pwhash 256B
        mkey = self.assist.mkkey(salt, pw, kf) # master key 48B
    ...
    def de(self, pw, kf, data, stpoint):
        ...
        if self.assist.svkey(salt, pw, kf) != pwhash:
...
class genfile: # kdm용 일반 모드 - file
    ...
    def en(self, pw, kf, hint, path):
        ...
        self.assist.inf2(fakeh + commonh + subtypeh + res + mhs + mh + chunksize, ckey, path, tgt)
        return tgt
    ...
    def de(self, pw, kf, path, stpoint):
        ...
        tgt = fopath + name 
        self.assist.inf3(stpoint, chunksize, ckey, path, tgt)
        return tgt
...
class funcfile: # kv용 기능 모드 - file
    ...
    def en(self, key, before, after):
        ...
        self.assist.inf2(ckeydt, ckey, before, after)
    ...
    def de(self, key, before, after):
        ...
        self.assist.inf3(1536, size, ckey, before, after)

# ========== ========== kaes5 end ========== ==========

class ffi:
    def __init__(self):
        self.os = "windows" # "windows" / "linux"
        if self.os == "windows":
            self.dll = "./kaeshy.dll"
            self.hvalue = b'\xd9C\xed\x00\xbd\xda\xfe\xa3\x16\x84W\x85$\x96p\x1f\x86\xa3\x9b\xf2\xdd\xf0\xd3/:k\x04z\xbc\xda\x10\x0b\x8b\x0f!\xa2\xe3\x07t\xa6?b\x1e=\xdd\xf0\xcb&\xdd\x14\x05#n}\xe4\xa2VE\xe4\xe8\xa1\x93\xce\xa4'
        else:
            self.dll = "./kaeshy.so"
            self.hvalue = b'\xe3\xb9\x1f`\x8a\xc7W/8\x9b\x82]\x8d~\x81\x04\xd5\xc5\\1\x18\x0fZv\x81\x06\xdd\xa29\x81\x06\xaf\x1f\xdcf\x1d\xf9\xc1\xe2t\xfdm\x1b8\xb8u\x8e\xcb\xc0\\l\x0fy\xb2\xfc\x8d\x7f\x170\xc0An6\x81'
        
        with open(self.dll, "rb") as f:
            temp = f.read()
        if hashlib.sha3_512(temp).digest() != self.hvalue:
            raise Exception("wrong ffi")

        if self.os == "windows":
            self.ext = ctypes.CDLL(self.dll)
        else:
            self.ext = ctypes.cdll.LoadLibrary(self.dll)

        self.ext.exf0.argtype = (ctypes.POINTER(ctypes.c_char),)
        self.ext.exf1.argtype = (ctypes.POINTER(ctypes.c_char), ctypes.c_int,
                                 ctypes.POINTER(ctypes.c_char), ctypes.c_int,
                                 ctypes.POINTER(ctypes.c_char), ctypes.c_int)
        self.ext.exf1.restype = ctypes.POINTER(ctypes.c_char)
        self.ext.exf2.argtype = (ctypes.POINTER(ctypes.c_char), ctypes.c_int,
                                 ctypes.POINTER(ctypes.c_char), ctypes.c_int,
                                 ctypes.POINTER(ctypes.c_char), ctypes.c_int)
        self.ext.exf2.restype = ctypes.POINTER(ctypes.c_char)
        self.ext.exf3.argtype = (ctypes.POINTER(ctypes.c_char), ctypes.c_int,
                                 ctypes.POINTER(ctypes.c_char),
                                 ctypes.POINTER(ctypes.c_char), ctypes.c_int,
                                 ctypes.POINTER(ctypes.c_char), ctypes.c_int)
        self.ext.exf4.argtype = (ctypes.c_int, ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char),
                                 ctypes.POINTER(ctypes.c_char), ctypes.c_int,
                                 ctypes.POINTER(ctypes.c_char), ctypes.c_int)

    def svkey(self, salt, pw, kf):
        pwa = ctypes.c_char_p(pw)
        pwl = len(pw)
        salta = ctypes.c_char_p(salt)
        saltl = len(salt)
        kfa = ctypes.c_char_p(kf)
        kfl = len(kf)

        ptr = self.ext.exf1(pwa, pwl, salta, saltl, kfa, kfl)
        out = [0] * 256
    ...
    def mkkey(self, salt, pw, kf):
        ...
        ptr = self.ext.exf2(pwa, pwl, salta, saltl, kfa, kfl)
        out = [0] * 48
        for i in range(0, 48):
            out[i] = ptr[i][0]
        self.ext.exf0(ptr)
    ...
    def inf2(self, header, ckey, before, after):
        heada = ctypes.c_char_p(header)
        headl = len(header)
        ckeya = ctypes.c_char_p(ckey)
        before = bytes(before, encoding="utf-8")
        beforea = ctypes.c_char_p(before)
        beforel = len(before)
        after = bytes(after, encoding="utf-8")
        aftera = ctypes.c_char_p(after)
        afterl = len(after)

        self.ext.exf3(heada, headl, ckeya, beforea, beforel, aftera, afterl)

    def inf3(self, stpoint, size, ckey, before, after):
        size = encode(size, 8)
        sizet = ctypes.c_char_p(size)
        ckeya = ctypes.c_char_p(ckey)
        before = bytes(before, encoding="utf-8")
        beforea = ctypes.c_char_p(before)
        beforel = len(before)
        ...
        self.ext.exf4(stpoint, sizet, ckeya, beforea, beforel, aftera, afterl)
