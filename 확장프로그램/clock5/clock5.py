...
class mainclass:
    def __init__(self):
        self.iswork, self.view = False, 0 # 백엔드 동작 여부 / 0, 1, 2 view
        self.istimer, self.timersec, self.timerstart = False, 600.0, 0.0 # 타이머 카운트 여부 / 타이머 남은 초 / 타이머 기준시점
        self.isstop, self.stopsec, self.stopstart = False, 0.0, 0.0 # 스탑워치 카운트 여부 / 스탑워치 기존 초 / 스탑워치 기준시점

    def gettime(self): # 시각, 날짜 반환
        dat = ['월요일', '화요일', '수요일', '목요일', '금요일', '토요일', '일요일']
        temp = time.time()
        ...
        return f"{ap} {hour:0>2}" + time.strftime(':%M:%S', now) + f".{msec}", time.strftime('%Y년 %m월 %d일 ', now) + dat[now.tm_wday]
    ...
    def backend(self): # working behind
        while not self.iswork:
            time.sleep(0.1)
        while self.iswork:
            if self.view == 0:
                t0, t1 = self.gettime()
            ...
            self.win.update()
            time.sleep(0.16) # update 6hz

    def mainfunc(self): # main gui
        self.win = tkinter.Tk()
        self.win.title("Clock5")
        self.win.geometry("370x200+300+200") # lxgp= 650x340+300+200
        self.win.resizable(False, False)

        notebook = tkinter.ttk.Notebook(self.win, width=350, height=160) # lxgp= w=640 h=280
        ...
        self.label0b.place(x=5, y=55) # lxgp= x=5 y=125
        ...
        lockb.place(x=5, y=115) # lxgp= x=5 y=205
        ...
        label1b.place(x=5, y=65) # lxgp= x=5 y=125
        ent1c = tkinter.Entry(fr1, font=("Consolas", 15), width=12)
        ent1c.place(x=135, y=65) # lxgp= x=230 y=125
        ...
        but1d.place(x=5, y=100) # lxgp= x=5 y=205
        ...
        but1e.place(x=65, y=100) # lxgp= x=95 y=205
        ...
        but1f.place(x=125, y=100) # lxgp= x=185 y=205
        ...
        list2b = tkinter.Listbox(fr2, width=17,  font=("맑은 고딕", 14), height=3) # lxgp= w=17 h=3 font=12
        list2b.place(x=5, y=60) # lxgp= x=5 y=125
        list2b.insert(1,'< 기록 >')
        ...
        but2c.place(x=185, y=95) # lxgp= x=320 y=205
        ...
        but2d.place(x=245, y=95) # lxgp= x=410 y=205
        ...
        but2e.place(x=295, y=95) # lxgp= x=500 y=205
