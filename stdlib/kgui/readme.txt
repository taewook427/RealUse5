...
<python reader>
class toolbox(str title, bool iswin, int mode)
    func entry()
    # 화면 초기화. current 설정 후 호출해야 함.
    func render()
    # 화면 그리기. current에 맞게 항목이 보여짐.
    func guiloop()
    # GUI 메인루프. 실행 시 GUI 창이 꺼질 때까지 루프를 돈다.
    func custom0(int x)
    # menu option[x] 항목 선택 시 실행되는 함수 틀. (x=0~11)
    func custom1(int x)
    # 사용자 버튼 클릭 시 실행되는 함수 틀. 왼쪽으로(x=0), 오른쪽으로(x=1), 새로고침(x=2), 특수버튼(x=3).
    func custom2(int x, int y)
    # 검색결과로 이동 시 실행되는 함수 틀. 이동위치가 [x][y]. 이동화면 렌더링 전 실행된다.

    .menus str[12] # 상단 메뉴버튼들의 옵션. 항상 12개 항목이 존재해야 한다.
    .big str[] # 대분류 목차들의 이름. 화면 상단 표시줄에 보여진다.
    .middle str[][] # 소분류 제목들. 화면 하단 표시줄에 보여진다.
    .small str/bytes[][] # 각 소분류 제목에 해당되는 문자열/사진 바이트열 내용들.
    .editable bool[3] # 각 입력창이 수정 가능한지 여부. True면 새로고침 버튼으로 수정된 데이터로 업데이트 가능.
    .current int[2] # 현재 보여지는 대분류-소분류 위치. 각 항목은 0 이상이여야 한다.
    .mwin TkObj # 메인 윈도우 창 TkInter 객체.
    .awin TkObj # 특수기능 윈도우 창 TkInter 객체.
...
kgui는 4개의 python 코드 파일을 제공한다. 사용하고자 하는 GUI 용도에 맞게 파일 중 하나를 선택해 import한다.
...
reader는 구조화된 텍스트/사진 문서를 읽고쓰는 상황에 사용한다.
특수버튼은 모드에 따라 사진뷰어(mode=0), 텍스트검색(mode=1), 현재상태확인(mode=2)로 동작한다.
사용 순서 : .menus/.big/.middle/.small -> entry -> .current/.editable -> guiloop
